...
function buildTemplateCsv() {
  return [
    "Brand,URL,Server IP,Username,Password,Note",
    'B200M,https://lp-example.com,,server-admin,Password123,"Main LP server"',
    'B200M2,https://brand-two.com,203.0.113.10,host-user,Password456,"Server IP is optional"',
  ].join("\n");
}
...
export default function LpServerBulkImportForm({
  busy,
  canUpdateExisting,
  onBack,
  onPreview,
  ...
  const handleDownloadTemplate = () => {
    ...
    link.href = objectUrl;
    link.download = "lp-servers-import-template.csv";
      ...
      <section className="app-panel management-form money-site-import-intro">
        ...
        <div className="money-site-import-editor-grid">
          <div className="management-field money-site-import-upload-card">
            <label htmlFor="lp-server-import-file">{copy.importForm.chooseFile}</label>
            <input
              id="lp-server-import-file"
              type="file"
              accept=".csv,text/csv"
          ...
          <div className="money-site-import-toggle-grid">
            <div className="management-field money-site-import-collapse-card">
              <div className="money-site-import-collapse-header">
                <div className="money-site-import-collapse-copy">
                  <label htmlFor="lp-server-import-text">{copy.importForm.csvContent}</label>
            ...
            <div className="management-field money-site-import-template-card money-site-import-collapse-card">
              <div className="money-site-import-collapse-header">
                <div className="money-site-import-collapse-copy">
                  <label htmlFor="lp-server-import-sample">{copy.importForm.templatePreview}</label>
                  <span className="money-site-import-collapse-meta">
              ...
              {showImportEditors ? (
                <div className="money-site-import-collapse-body">
                  <textarea id="lp-server-import-sample" value={templateCsv} readOnly rows={16} />
                </div>
              ) : (
                <div className="money-site-import-collapse-preview">
          ...
          {hasPreview ? (
            <div className="money-site-import-filter-bar">
              <div className="management-search">
                <label htmlFor="lp-server-import-search">{copy.importForm.searchPreview}</label>
                <input
                  id="lp-server-import-search"
                  type="search"
                  value={previewSearch}
          ...
          {hasPreview && filteredRows.length ? (
            <>
                ...
                <table className="brands-management-table money-site-import-table">
                  ...
                  <tbody>
                    {filteredRows.map((row) => (
                      <tr
                        key={`row-${row.rowNumber}-${row.brandName}-${row.url}`}
                      ...
                      >
                        ...
                        <td>
                          <div className="management-stack money-site-import-record-cell">
                            ...
                            <span className="money-site-import-subline">{`${copy.importForm.fieldLabels.serverIp}: ${formatOptionalField(row.serverIp)}`}</span>
                            <span className="money-site-import-subline">{`${copy.importForm.fieldLabels.username}: ${formatOptionalField(row.username)}`}</span>
                            <span className="money-site-import-subline">{`${copy.importForm.fieldLabels.password}: ${formatOptionalField(row.passwordPreview)}`}</span>
                            <span className="money-site-import-subline">{`${copy.importForm.fieldLabels.note}: ${formatOptionalField(row.note)}`}</span>
                          </div>
                        </td>
                        <td>
                          <div className="management-stack money-site-import-record-cell">
                            <strong>{row.existing?.url || copy.importForm.noExistingRecord}</strong>
                            {row.existing ? (
                              <>
                                <span className="money-site-import-subline">{`${copy.importForm.fieldLabels.brand}: ${row.existing.brandName || "-"}`}</span>
                                <span className="money-site-import-subline">{`${copy.importForm.fieldLabels.serverIp}: ${formatOptionalField(row.existing.serverIp)}`}</span>
                                <span className="money-site-import-subline">{`${copy.importForm.fieldLabels.username}: ${formatOptionalField(row.existing.username)}`}</span>
                                <span className="money-site-import-subline">{`${copy.importForm.fieldLabels.password}: ${formatOptionalField(row.existing.passwordPreview)}`}</span>
              ...
              <div className="money-site-import-mobile-list">
                {filteredRows.map((row) => (
                  <article
                    key={`mobile-${row.rowNumber}-${row.brandName}-${row.url}`}
                  ...
                  >
                    ...
                    <div className="money-site-import-mobile-grid">
                      <div className="money-site-import-mobile-block">
                        ...
                        <small>{`${copy.importForm.fieldLabels.serverIp}: ${formatOptionalField(row.serverIp)}`}</small>
                        <small>{`${copy.importForm.fieldLabels.username}: ${formatOptionalField(row.username)}`}</small>
                        <small>{`${copy.importForm.fieldLabels.password}: ${formatOptionalField(row.passwordPreview)}`}</small>
                      ...
                      <div className="money-site-import-mobile-block">
                        ...
                        {row.existing ? (
                          <>
                            ...
                            <small>{`${copy.importForm.fieldLabels.serverIp}: ${formatOptionalField(row.existing.serverIp)}`}</small>
                            <small>{`${copy.importForm.fieldLabels.username}: ${formatOptionalField(row.existing.username)}`}</small>
                            <small>{`${copy.importForm.fieldLabels.password}: ${formatOptionalField(row.existing.passwordPreview)}`}</small>
      ...
      {showConfirmModal ? (
        <div className="management-modal-backdrop is-centered" role="presentation">
          <form
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="lp-server-import-confirm-title"
          >
            <div className="management-section-header">
              <div>
                <h2 id="lp-server-import-confirm-title">{copy.importForm.confirmTitle}</h2>
